...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("How fast the player shoul dmove when running around.")]
    private float m_Speed;

    [SerializeField]
    [Tooltip("Thet ransform of the caemera follwing the player")]
    ...
    [SerializeField]
    [Tooltip("Amount of health that the poayer starts with")]
    ...
    //In order to do naything, we cannot be frozen (timer must be 0)
    ...
    private void Awake()
    {
        ...
        for (int i=0; i < m_Attacks.Length; i++)
        {
            ...
            {
                UnityEngine.Debug.LogError(attack.AttackName + "has a wind up time that is longer than the amount of time the player is frozen for");
            }
        }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            IncreaseHealth(other.GetComponent<HeatlhPill>().HealthGain);
